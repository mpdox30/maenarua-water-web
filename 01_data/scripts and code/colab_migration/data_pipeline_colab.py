...
def _wd_run_prediction(model: dict, features: dict) -> tuple[Optional[dict], Optional[dict]]:
    ...
    2026-09-21 แก้ -- sync กับ data_pipeline.py ตัวจริง (แก้บั๊กเดียวกันที่นั่นก่อนแล้ว): เดิม hardcode
    h=1 อย่างเดียว ทั้งที่ catboost_models.pkl/lightgbm_models.pkl/stage1_classifiers.pkl/
    stack_weights.pkl มี key (zone, h) ครบ h=1..12 อยู่แล้วทั้ง 2 โซน แปลว่า h2-h12 เทรนเสร็จแต่ไม่เคย
    ถูกเรียกใช้จริง -- วนลูปแบบเดียวกับ _ri_run_prediction() ตอนนี้ ถ้า horizon ไหนโมเดลไม่ครบ (ไม่ควร
