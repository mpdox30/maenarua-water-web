...
======================
**Colab migration variant ของ era5t_worker.py (ต้นฉบับอยู่ที่
01_data/scripts and code/pipeline/era5t_worker.py — ห้ามแก้ไฟล์เดิม)**

Business logic (สูตร Penman-Monteith FAO-56, ค่าคงที่ ELEV_M/ALPHA/MJ_DAY, การดึงข้อมูลที่
time=12:00 UTC สำหรับโหมดรายสัปดาห์, retry-shrink logic ใน _fetch_week_with_retry(), การรวม
2 datasets จาก cfgrib.open_datasets() เข้าด้วยกัน) **เหมือนต้นฉบับทุกจุด ไม่มีการแก้ไข**

สิ่งที่ต่างจากต้นฉบับ — เฉพาะส่วนที่เกี่ยวกับ environment เท่านั้น:
- ต้นฉบับต้องแยกเป็นสคริปต์ต่างหาก เพราะ environment ที่ import cdsapi/cfgrib/eccodes ได้สำเร็จ
  (conda-forge stack ที่มากับ ArcGIS Pro บน Windows) เป็นคนละ Python interpreter กับ .venv หลัก
  ของ data_pipeline.py — ต้องเรียกผ่าน subprocess ข้าม environment
- บน Colab (Linux): ยืนยันแล้วว่า `pip install cdsapi cfgrib eccodes ecmwflibs xarray` ติดตั้ง
  eccodes ได้สำเร็จตรงๆ ใน environment เดียวกับส่วนอื่นของ pipeline ("Cannot find the ecCodes
  library" เป็นปัญหาเฉพาะ pip-on-Windows เท่านั้น) — จึงไม่จำเป็นต้องมี subprocess ข้าม
  environment อีกต่อไป ฟังก์ชันในไฟล์นี้เรียกตรงได้จากโปรเซสเดียวกันเลย
- ตอนนี้ (ขั้นทดสอบ) ยังคง**คง CLI interface เดิมทั้งหมดไว้** (argparse, --out-json ฯลฯ) เพื่อให้
  รันเทียบผลลัพธ์กับ era5t_worker.py ต้นฉบับได้ตรงๆ ก่อนตัดสินใจว่าจะเปลี่ยนเป็น import ตรง
  (ไม่ผ่าน CLI/subprocess) ในขั้นต่อไปที่จะเชื่อมเข้า pipeline เต็มรูปแบบหรือไม่ — path ทั้งหมด
  ในไฟล์นี้ใช้ pathlib.Path อยู่แล้วตั้งแต่ต้นฉบับ (cross-platform) จึงไม่มี path-separator
  ที่ต้องแก้เพิ่ม

Contract กับผู้เรียก (เดิม, คงไว้เพื่อ backward-compat ตอนทดสอบ):
...
def parse_args(argv=None) -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description="ดึง + decode ERA5T single-levels (Colab variant — เดิมเรียกโดย data_pipeline.py ผ่าน subprocess)"
    )
...
def _iso_week_days(as_of: date, today: Optional[date] = None) -> list:
    ...
    ซึ่งเป็นวันอาทิตย์ของสัปดาห์นั้นไปเฉยๆ) พอร์ตมาจาก pipeline/era5t_worker.py ตัวจริง
...
def _run_weekly_mode(args, out_json_path: Path, result: dict) -> tuple:
    ...
    # ---------------------------------------------------------------------------
    # 2026-07-22 เพิ่ม (พอร์ตมาจาก era5t_worker.py ต้นฉบับ — logic เหมือนกันทุกประการ): rolling
    # 7-day window fallback เมื่อสัปดาห์ปฏิทิน ISO ของ as_of เอง**ไม่มีข้อมูลจริงเลยสักวัน**
    # (n_days == 0) — เกิดขึ้นได้เสมอต้นสัปดาห์เพราะ ERA5T (CDS) มี latency ~5 วัน (พบจริงจาก log
    # Colab 2026-07-22: as_of=2026-07-22 (พุธ), candidate_days=[07-20, 07-21] ถูก CDS ปฏิเสธทั้งคู่
    # เพราะข้อมูลล่าสุดที่มีจริงคือ 07-17) ติดแท็ก is_rolling_estimate=true ให้ผู้เรียก
    # (data_pipeline_colab.py) แยกแยะได้ — **สำคัญ**: ห้ามป้อนค่านี้เข้าโมเดล Water Demand โดยตรง
    # (out-of-distribution เทียบกับตอน train) ใช้ได้แค่เป็น advisory metric (AI_week) เท่านั้น
