...
ว่า "ยังไม่ต้องเชื่อมเข้า data_pipeline.py จริง แค่ทดสอบฟังก์ชัน parse ก่อน":**
  ...
  5. **ความถี่จริงที่สังเกตได้ (ยืนยันด้วยการเรียกจริงหลายรอบ ห่างกันจริงตามเวลานาฬิกา ไม่ใช่แค่
     ...
     **นี่ไม่ใช่แค่คำถามเรื่อง "10 นาทีหรือ 30 นาที" แต่เป็นสัญญาณว่า response ไม่ monotonic จริง**
     ...
     ข้อมูลรอบใหม่แบบไม่เช็คก่อนเด็ดขาด ต้องเทียบ measure_datetime ใหม่กับล่าสุดที่เก็บไว้เสมอ**
     (หมายเหตุ 2026-07-30: telemetry_history_store.py ที่เคย implement การเช็คนี้แบบ standalone
     ถูกลบทิ้งแล้ว เพราะไม่เคยต่อเข้ากับ pipeline จริง — แหล่งข้อมูลย้อนหลังที่ใช้งานจริงตอนนี้คือ
     wide_log Google Sheet ผ่าน reservoir_telemetry_from_sheet.load_wide_log() ซึ่งคืนค่าทุกแถวที่
     สะสมไว้ ให้ผู้เรียกกรอง/เช็ค monotonic เองตามต้องการ) ยังไม่ได้วิเคราะห์ข้อมูลต่อเนื่องนานพอ
     ...
     สะสมพอหรือได้เอกสาร API อย่างเป็นทางการจาก กช.สสน.
