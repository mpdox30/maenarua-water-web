...
# 2026-09-23 เพิ่ม -- prediction interval สำหรับ Water Demand
# ===================================================================================
# ลองมา 2 วิธี:
#   1) Mondrian + Normalized conformal (Variant D, combined_final_pipeline.ipynb Step 4v3) --
#      conformal_intervals.json (build_conformal_intervals.py) -- ยังเก็บไฟล์ไว้เป็น reference/
#      เปรียบเทียบ แต่ **เลิกใช้ใน production แล้ว** เพราะ Zone A ยังกว้างเกินไปแม้ขยาย calibration
...
# (แก้ไข 2026-09-17 -- ตัดสินใจร่วมกับผู้ใช้: เหตุการณ์น้ำล้นจริงตั้งแต่ 11 ก.ย. 2569 ทำให้ %Full_t
# ขึ้นไปแตะ 108.1% (จุดสูงสุด 11 ก.ย.) และยังคง >105% ต่อเนื่อง 6 วัน (ถึง 16 ก.ย. อยู่ที่ 105.05%)
# เกณฑ์เดิม 105% เลยกันข้อมูลช่วงน้ำล้นจริงทั้งหมดไม่ให้เป็น "แถวล่าสุดที่ใช้ได้" ของโมเดล ทำให้
# as_of_date ค้างที่ 10 ก.ย. ไม่ขยับ -- ขยับเพดานเป็น 120% (เผื่อรับเหตุการณ์ระดับใกล้เคียงหรือสูงกว่า
# นี้เล็กน้อยได้ต่อเนื่อง โดยยังกันข้อมูลผิดพลาดร้ายแรงจริงๆ เช่น sensor error ที่พุ่งเกินกว่านี้มาก)
# ผลข้างเคียงที่ต้องรู้: ช่วง %Full_t 105-120% เป็นช่วงที่โมเดลไม่เคยเห็นตอนเทรน (out-of-distribution)
# ค่าพยากรณ์ในช่วงนี้จึงอาจแม่นยำน้อยกว่าปกติ -- ผู้ใช้ยืนยันแล้วว่าต้องการให้ระบบทำงานต่อเนื่องเพื่อ
# เก็บข้อมูลจริงไว้ปรับแก้/retrain โมเดลในอนาคต มากกว่าจะหยุดพยากรณ์ไปเฉยๆ ระหว่างเหตุการณ์สำคัญนี้
...
# ---------------------------------------------------------------------------
# Water Demand -- Kc target-week feature (deploy 2026-09-23, ดู #52-57)
# ---------------------------------------------------------------------------
...
def _wd_find_area_source_for_date(as_of_date) -> tuple:
    """
    2026-08-20 เพิ่ม -- ย้ายมาจาก backfill_nir_gir_weeks.py::_find_area_source() ให้เป็นจุดเดียว
    (single source of truth) เพราะตอนนี้มี 2 จุดที่ต้องหา area basis "ตามช่วงเวลาจริง" ของสัปดาห์
    ย้อนหลัง: สคริปต์ backfill_nir_gir_weeks.py (manual, on-demand) และ
    _backfill_incomplete_climate_weeks() (อัตโนมัติ ทุกรอบ pipeline) -- ถ้าปล่อยให้มีสูตร/logic
    หาไฟล์ SAR แยกกัน 2 ชุด เสี่ยงหลุด sync กันแบบเดียวกับที่กันไว้แล้วสำหรับ _wd_compute_live_nir_gir()
    (ดู docstring ฟังก์ชันนั้น + comment ใน backfill_nir_gir_weeks.py)

    หลักการเดียวกับ _wd_get_area_zone_ha(): ต้องใช้ผล SAR classification ที่ "active อยู่จริง ณ
    as_of_date" ไม่ใช่ผลล่าสุดวันนี้ (กัน look-ahead bias) -- สแกนไฟล์ dated audit-trail
...
def _backfill_incomplete_climate_weeks(
    ...
    **2026-08-20 แก้บั๊ก**: เดิมฟังก์ชันนี้ปล่อย NIR_A_m3/GIR_B_m3 เป็น None เสมอ (เหตุผลเดิม: "ต้องใช้
    พื้นที่ SAR classification ณ ตอนนั้นซึ่งอาจต่างจากปัจจุบัน" — ตอนเขียนแรกๆ live path ยังไม่เรียกใช้
    ค่านี้จริง เลยมองว่าไม่กระทบ) หลัง Phase 3 (live-wire FAO-56 NIR/GIR เข้า Water Demand pipeline)
    ค่านี้ถูกใช้จริงแล้ว ทำให้แถว blank ที่ฟังก์ชันนี้ append ไปทับแถว "ok" เดิมเงียบๆ ทุกครั้งที่ backfill
    สัปดาห์ซ้ำ (dedup ยึด run_timestamp ล่าสุด) — เกิดจริง 2 ครั้ง (สัปดาห์ 30, 31 เดือน ส.ค. 2026)
    ตอนนี้คำนวณ NIR/GIR ให้เสมอเมื่อ climate ครบ 7/7 โดยใช้ area basis "ตามช่วงเวลาจริง" ของสัปดาห์นั้น
    ผ่าน `_wd_find_area_source_for_date()` (จุดเดียวกับที่ `backfill_nir_gir_weeks.py` ใช้ — กัน
    look-ahead bias เหมือนเดิมทุกประการ ไม่ได้ผ่อนหลักการนี้ลง) ถ้าคำนวณไม่สำเร็จ (area/kc ไม่พร้อม)
    จะ fallback เป็นแถว blank เหมือนพฤติกรรมเดิม ไม่ raise
    ...
    for zone, year_i, week_i in candidate_keys:
        ...
        if chirps_days == 7 and era5t_days == 7 and not chirps_is_rolling and not era5t_is_rolling:
            ...
            # 2026-08-20 เพิ่ม -- แก้บั๊ก: เดิมฟังก์ชันนี้ปล่อย NIR_A_m3/GIR_B_m3 เป็น None เสมอ (ดู
            # docstring หัวข้อ "ข้อจำกัดของแถวที่ backfill" ด้านบน) ซึ่งตอนนั้นไม่กระทบเพราะ live path
            # ยังไม่เรียกใช้ค่าพวกนี้จริง แต่หลัง Phase 3 (live-wire FAO-56 NIR/GIR) ค่านี้ถูกใช้จริงแล้ว
            # ทำให้ทุกครั้งที่ฟังก์ชันนี้ backfill สัปดาห์ที่เคยมีแถว "ok" (คำนวณ NIR/GIR ไว้แล้ว) ซ้ำ
            # (เช่น climate ของสัปดาห์นั้นไม่ครบ 7/7 ตอนแรก แล้ว provider ค่อยปล่อย final ทีหลัง) แถวใหม่
            # (blank) จะไปทับแถว "ok" เดิมเงียบๆ เพราะ dedup ยึด run_timestamp ล่าสุด (พบจริง 2 ครั้ง:
            # สัปดาห์ 30 เดือน ส.ค. และสัปดาห์ 31 -- แก้ด้วย backfill_nir_gir_weeks.py ทุกครั้งที่เจอ)
            # แก้ที่ต้นตอตรงนี้แทน: คำนวณ NIR/GIR ให้เสมอเมื่อ climate ครบ 7/7 แล้ว โดยใช้ area basis
            # "ตามช่วงเวลาจริง" ของสัปดาห์นั้นผ่าน _wd_find_area_source_for_date() เดียวกับที่
            # backfill_nir_gir_weeks.py ใช้ (single source of truth เดียวกัน กัน logic หลุด sync)
            nir_a_m3 = None
            gir_b_m3 = None
            wd_area_basis = None
...
def _wd_run_prediction(model: dict, features: dict) -> tuple[Optional[dict], Optional[dict]]:
    ...
    2026-09-23 แก้ (deploy Kc feature, ดู #52-57) -- catboost_models.pkl/lightgbm_models.pkl/
    stack_weights.pkl (Stage2 magnitude regressor) ตอนนี้เป็นเวอร์ชัน train ด้วย kc_target_h{h}
    เพิ่มเข้ามาต่อ horizon แล้ว (37 base features + 1 Kc = 38) ต้องต่อ X_reg เดิม (37 ตัว, มาจาก
    _wd_build_feature_vector()/_wd_extract_feature_row_for_zone() เหมือนเดิมทุกประการ) เข้ากับค่า Kc
    ของสัปดาห์เป้าหมาย (as_of + h) คำนวณสดผ่าน _wd_kc_target_h() ก่อนป้อนเข้า cat_model/lgb_model
    เท่านั้น -- **Stage1 classifier (X_clf) และ CQR quantile models (cqr_quantile_models, ยังไม่ได้
    retrain ด้วย Kc) ยังคงใช้ X_reg เดิมแบบไม่มี Kc เหมือนเดิมทุกประการ ห้ามป้อน X_reg_kc ให้สองตัวนี้
    เด็ดขาด (จำนวน feature จะไม่ตรงกับตอน train แล้วพัง)** -- หมายเหตุสำคัญที่ยังไม่แก้ในรอบนี้: ช่วง
    ความเชื่อมั่น (lower_m3/upper_m3) คำนวณจาก residual ของโมเดลเดิม (ไม่มี Kc) ตอนนี้โมเดลจุด
    เปลี่ยนไปแล้วแต่ interval ยังไม่ได้ calibrate ใหม่ -- ตัวเลข final_m3 ถูกต้องตามโมเดลใหม่ แต่ความ
    กว้างของ interval อาจไม่สะท้อน error จริงของโมเดลใหม่เป๊ะ (ต้องรัน build_mondrian_cqr_intervals.py
    ใหม่ในรอบถัดไปถ้าต้องการ interval ที่ calibrate ตรงกับโมเดล Kc)
    ...
    for zone in ("zone_A", "zone_B"):
        ...
        for h in range(1, WD_HORIZON + 1):
            ...
            # Kc target-week feature ต่อ h เฉพาะสำหรับ Stage2 magnitude regressor (cat/lgb) เท่านั้น
            # -- ดู docstring ด้านบน ห้ามใช้กับ clf/cqr ที่ยังเป็นสเปก feature เดิม (ไม่มี Kc)
            try:
                kc_val = _wd_kc_target_h(zone, feat["as_of_year"], feat["as_of_week"], h)
            except Exception:
                logger.exception(
                    "คำนวณ Kc target-week feature ล้มเหลว (%s h%d) -- ใช้ 0.0 แทน (อาจทำให้ magnitude "
                    "ไม่แม่นเท่าที่ควรรอบนี้ แต่ไม่ควร error ทั้ง pipeline)", zone, h,
                )
                kc_val = 0.0
...
def _ri_run_prediction(model: dict, features: dict) -> dict:
    ...
    for h, target_col in zip(horizons, targets):
        ...
        # single-split test_nse proved noisy/unstable across this project's
