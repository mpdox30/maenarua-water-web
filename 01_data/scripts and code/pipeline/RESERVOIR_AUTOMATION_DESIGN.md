...
4. **ตั้ง env var `RESERVOIR_TELEMETRY_API_URL` บนเครื่องที่จะรัน scheduled task จริง** (ตอนนี้
   ...
   หมายเหตุ 2026-07-18: env var อีกตัวที่เกี่ยวข้อง — `RESERVOIR_RELEASE_SHEET_CSV_URL` (สำหรับ
   ดึง `release_log`) — ตั้งเสร็จแล้วบนเครื่อง Windows ที่ใช้รัน orchestration จริง (ดูหัวข้อ
   "Publish-to-web + ตั้ง env var" ท้ายไฟล์)
...
| 2.6 | `พื้นที่รับน้ำ (ถ้าเพื่อการเกษตร)` | Short answer | ไม่ required — เปลี่ยนชื่อคำถามจาก draft เดิม "หมายเหตุ (เปิด)" ตอนสร้าง Form จริง (ตัดสินใจไว้ก่อน 2026-07-18) เพื่อให้เจาะจงประโยชน์การใช้งานมากขึ้น ไม่กระทบการคำนวณ Inflow |
...
var ITEM_TITLES = {
  ...
  OPEN_NOTE: 'พื้นที่รับน้ำ (ถ้าเพื่อการเกษตร)', // ชื่อจริงใน Form ต่างจาก draft เดิม "หมายเหตุ (เปิด)" — ดูหัวข้อ 2.6 ด้านบน
...
/**
 * แปลง e.response (FormResponse) เป็น namedValues-like map { title: [answer] }
 * ทำไมต้องมีฟังก์ชันนี้: trigger ที่ผูกกับ **Form** (From form / On form submit) ส่ง
 * e.response มา ไม่ใช่ e.namedValues (รูปแบบนั้นมีแค่ trigger ที่ผูกกับ **Sheet** เท่านั้น)
 * ถ้าอ่าน e.namedValues ตรงๆ กับ Form-trigger จะได้ undefined เงียบๆ ไม่ throw error
 * (นี่คือ root cause ของบั๊ก "submit ปิดวาล์วแล้ว end_date/end_time ไม่อัปเดต" ที่เจอ 2026-07-18)
 *
 * .trim() ตอน set key ก็สำคัญเช่นกัน: ถ้าชื่อคำถามจริงใน Form มี space เกินมา (พิมพ์มือ)
 * การ lookup ด้วย ITEM_TITLES ที่สะกดสะอาดจะไม่ match แล้วได้ '' แบบเงียบๆ อีกแบบ (เจอจริง
 * ในเซสชันนี้กับคำถามฝั่งปิดวาล์ว 3 ข้อ)
 */
function responseToNamedValues_(formResponse) {
  var nv = {};
  var itemResponses = formResponse.getItemResponses();
  for (var i = 0; i < itemResponses.length; i++) {
    var ir = itemResponses[i];
    var title = ir.getItem().getTitle();
    nv[title.trim()] = [ir.getResponse()];
  }
  return nv;
}
...
function onFormSubmitReleaseLog(e) {
  try {
    var nv = responseToNamedValues_(e.response);
    ...
    if (action === ACTION_OPEN) {
      ...
      console.log('onFormSubmitReleaseLog: action ไม่รู้จัก: ' + action);
    }
    // อัปเดต dropdown ปิดวาล์วทันทีหลัง submit แต่ละครั้ง (ไม่ต้องรอ trigger 15 นาที)
    refreshCloseDropdownChoices();
  } catch (err) {
    console.log('onFormSubmitReleaseLog ล้มเหลว: ' + err + ' | stack=' + err.stack);
    // ไม่ throw ต่อ -- กัน Form response หายไปเงียบๆ ถ้าอยากได้ alert อีเมลตอน error จริง
    // เพิ่ม MailApp.sendEmail(...) ตรงนี้ได้
    //
    // หมายเหตุ: ใช้ console.log (ไม่ใช่ Logger.log) เพราะ Logger.log ไม่โผล่ใน Executions log
    // ของ execution ที่ trigger เรียกเอง (ต่างจากรันมือ) อย่างน่าเชื่อถือ -- console.log โผล่ชัวร์กว่า
  }
}
...
function handleCloseEvent_(nv) {
  ...
  if (!m) {
    console.log('handleCloseEvent_: หา event_no จาก dropdown label ไม่เจอ: ' + selected);
    return;
  }
  ...
  console.log('handleCloseEvent_: ไม่พบ event#' + eventNo + ' ที่ยังเปิดอยู่ (อาจถูกปิดไปแล้วก่อนหน้า)');
}
...
function refreshCloseDropdownChoices() {
  ...
  for (var i = 1; i < data.length; i++) {
    ...
    if (eventNo !== '' && (!endDate || endDate === '')) {
      labels.push('event#' + eventNo + ' - ' + data[i][5] + ' - เปิดเมื่อ ' +
        formatDateCell_(data[i][1]) + ' ' + formatTimeCell_(data[i][2]));
    }
  }
  if (labels.length === 0) {
  ...
  console.log('refreshCloseDropdownChoices: หาคำถามชื่อ "' + ITEM_TITLES.CLOSE_EVENT + '" ใน Form ไม่เจอ');
}
```

**อัปเดตสำคัญ 2026-07-18 (รอบทดสอบจริง)**: โค้ดด้านบนคือเวอร์ชันที่ทดสอบผ่านจริงแล้ว (submit
"เปิดวาล์วใหม่" แล้ว "ปิดวาล์วที่เปิดอยู่" ครบ end-to-end, ตรวจสอบแถวใน `release_log` ตรงตามคาด)
ต่างจากโค้ดฉบับแรกที่วางไว้ตอนสร้าง Form (ก่อนเจอบั๊ก) ที่จุดสำคัญ 2 จุด — ถ้า copy Code.gs ไปวางใหม่
ให้ใช้เวอร์ชันนี้ ไม่ใช่เดารายละเอียดจากคอมเมนต์เก่า:
- `e.namedValues` → ต้องเปลี่ยนเป็น `responseToNamedValues_(e.response)` (Form-bound trigger ไม่มี
  `e.namedValues`, มีแต่ `e.response`)
- เพิ่ม `.trim()` ตอน build key ของ namedValues map เพื่อกันปัญหาชื่อคำถามใน Form มี
  whitespace เกินมาโดยไม่ตั้งใจ (พิมพ์มือ)

## Publish-to-web + ตั้ง env var — ✅ ทำเสร็จแล้ว (2026-07-18)
...
3. Copy ลิงก์ที่ได้ (รูปแบบ `https://docs.google.com/spreadsheets/d/e/<id>/pub?gid=<gid>&single=true&output=csv`)
   — ลิงก์จริงที่ publish แล้ว:
   `https://docs.google.com/spreadsheets/d/e/2PACX-1vQQiTQdJo0H3XAaqZwikW51JfccDS1J8Ot_vnpqd-nh-AwM19dPhhBidk3kT1nFGoRyrZLvuMepBDI-/pub?gid=1786583317&single=true&output=csv`
4. บนเครื่อง Windows ที่รัน `reservoir_daily_orchestration.py` เปิด Command Prompt แล้วรัน
   (ครั้งเดียว ค่าจะติดถาวรกับ user account) — **ตั้งแล้ว**:
   ```
   setx RESERVOIR_RELEASE_SHEET_CSV_URL "<ลิงก์ที่ copy มา>"
   ```
   หมายเหตุการตรวจสอบ: `echo %RESERVOIR_RELEASE_SHEET_CSV_URL%` แบบไม่มี quote จะ error
   (`'single' is not recognized...` ฯลฯ) เพราะ cmd.exe ตีความ `&` ในลิงก์ (หลัง expand ตัวแปร)
   เป็นตัวแบ่งคำสั่ง — ไม่ใช่ค่าตัวแปรเสียหาย ให้ตรวจสอบด้วย `echo "%RESERVOIR_RELEASE_SHEET_CSV_URL%"`
   (มี quote ล้อมตัวแปร) หรือ PowerShell `$env:RESERVOIR_RELEASE_SHEET_CSV_URL` แทน
