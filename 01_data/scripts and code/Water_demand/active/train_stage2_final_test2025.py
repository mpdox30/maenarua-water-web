"""
train_stage2_final_test2025.py
================================
2026-09-23 -- ทดลองตามคำขอ: "โมเดลสุดท้าย" (Kc + 2020-2025 data) แต่เปลี่ยน
year split เป็น TRAIN=[2020,2021,2022,2023], CALIB=2024, TEST=[2025]
(ใช้ประโยชน์จากข้อมูลปี 2025 ที่ backfill มาเป็น TEST จริง ไม่ใช่แค่เติม
lag/rolling ของปลายปี 2024 เหมือนก่อนหน้านี้)

ปีทั้งสามชุด (TRAIN/CALIB/TEST) แยกจากกันสนิท ไม่ overlap -- ไม่มี leakage
...
หมายเหตุ: ปี 2025 เป็นปีสุดท้ายที่มีแค่ 51 สัปดาห์ (backfill ได้ถึง week 51)
และยิ่ง horizon สูง (h ใกล้ 12) จำนวนแถว test ยิ่งลดลง เพราะ y_h{h} =
shift(-h) ของ target ที่ต้องการข้อมูล "อนาคต" ซึ่งเกินขอบเขตของปี 2025
ที่มีอยู่ -- เป็นข้อจำกัดโดยธรรมชาติของการใช้ปีล่าสุดเป็น TEST ไม่ใช่บั๊ก

Output (ไฟล์อ้างอิง ยังไม่ deploy):
  catboost_models_final_baseline.pkl / lightgbm_models_final_baseline.pkl / stack_weights_final_baseline.pkl
  catboost_models_final_kc.pkl       / lightgbm_models_final_kc.pkl       / stack_weights_final_kc.pkl
"""
import warnings
...
def train_one(df, kc_df, use_kc: bool, tag: str):
    ...
    for zone in ZONES:
        ...
        for h in range(1, HORIZON + 1):
            ...
            if len(X_train) == 0 or len(X_test) == 0:
                print(f"[skip] {tag} {zone} h{h}: n_train={len(X_train)} n_test={len(X_test)} -- ข้ามเพราะข้อมูลไม่พอ")
                continue
    ...
    joblib.dump(cat_models, ACTIVE_DIR / f"catboost_models_final_{tag}.pkl")
    joblib.dump(lgb_models, ACTIVE_DIR / f"lightgbm_models_final_{tag}.pkl")
    joblib.dump(weights, ACTIVE_DIR / f"stack_weights_final_{tag}.pkl")
...
def main():
    ...
    print("\n" + "=" * 90)
    print("SUMMARY -- TRAIN=2020-2023, CALIB=2024, TEST=2025 -- baseline vs +Kc (apples-to-apples)")
    print("=" * 90)
    for zone in ZONES:
        b = edf_base[edf_base["zone"] == zone]
        k = edf_kc[edf_kc["zone"] == zone]
        # เทียบเฉพาะ horizon ที่มีทั้งคู่ (n_test เท่ากันไหม เช็คด้วย)
    ...
    print("\nSaved: catboost/lightgbm/stack_weights_final_{baseline,kc}.pkl (ไฟล์อ้างอิง ยังไม่ deploy)")
